...
pygame.display.set_caption("Space Shooter - Animated Player")
clock = pygame.time.Clock()

PLAYER_W, PLAYER_H = 45, 90
...
# ━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
#  스프라이트 시트 Base64 데이터 (image_test.py 에서 가져옴)
#내 base 64 넣을거
# ━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
SHEET_B64 = ""

sheet_bytes = base64.b64decode(SHEET_B64)
player_sheet_raw = pygame.image.load(io.BytesIO(sheet_bytes)).convert_alpha()

# image_test.py 설정값
S_FRAME_W, S_FRAME_H = 334, 768
S_COLS = 4
S_FRAME_DELAY = 150 # ms

player_frames = []
for i in range(4):
    row, col = divmod(i, S_COLS)
    rect = pygame.Rect(col * S_FRAME_W, row * S_FRAME_H, S_FRAME_W, S_FRAME_H)
    frame = player_sheet_raw.subsurface(rect)
    # 게임 크기에 맞게 미리 스케일 조정 (PLAYER_W, PLAYER_H)
    player_frames.append(pygame.transform.scale(frame, (PLAYER_W, PLAYER_H)))
...
class Boss:
    ...
    def draw(self, surf):
        if BOSS_IMAGE:
            surf.blit(BOSS_IMAGE, self.rect.topleft)
        else:
...
# 드로잉 유틸
# ─────────────────────────────────────────────
def draw_player(surf, rect, frame_index, state="idle"):
    # 애니메이션 프레임 선택
    base_img = player_frames[frame_index]
...
def draw_hud(score, hp, max_hp, drone_count, drone_spread, coins, crystals):
    ...
    cheat_txt = "[F1:Item] [F2:Drone] [F3:Inst Drone] [F4:Clear]"
...
def main():
    ...
    while True:
        ...
        for e in pygame.event.get():
            ...
            if e.type == pygame.KEYDOWN:
                if e.key == pygame.K_F1:
                    ...
                    items.append({'rect': pygame.Rect(player.centerx, player.centery - 100, ITEM_SIZE, ITEM_SIZE), 'type': itype, 'color': icolor})
                if e.key == pygame.K_F2:
                    items.append({'rect': pygame.Rect(player.centerx, player.centery - 100, ITEM_SIZE, ITEM_SIZE), 'type': 'drone', 'color': PURPLE})
                ...
                if e.key == pygame.K_F5: score += 500
                if e.key == pygame.K_F6: coins_earned+=100
                if e.key == pygame.K_F7: crystals_earned+=1

        # --- 애니메이션 업데이트 ---
        p_frame_timer += dt
        if p_frame_timer >= S_FRAME_DELAY:
            p_frame_index = (p_frame_index + 1) % len(player_frames)
            p_frame_timer = 0
        ...
        if (invincible // 5) % 2 == 0: 
            draw_player(screen, player, p_frame_index, state=player_state)
